...
class TestLofFullStackSemantics:
    ...
    def test_tushare_recognizes_lof(self):
        """TushareFetcher must recognize LOF codes and route to .SH."""
        from data_provider.tushare_fetcher import _is_etf_code as ts_etf, TushareFetcher
        assert ts_etf("501018"), "Tushare should recognize 501018 as fund"
        assert ts_etf("161116"), "Tushare should recognize 161116 as fund"
        assert not ts_etf("180003"), "Tushare should not recognize 180003"

    def test_baostock_routes_lof_to_sh(self):
        """BaostockFetcher must route LOF 501/502/506 to sh. prefix."""
        from data_provider.baostock_fetcher import BaostockFetcher
        fetcher = BaostockFetcher.__new__(BaostockFetcher)
        assert fetcher._convert_stock_code("501018") == "sh.501018"
        assert fetcher._convert_stock_code("502000") == "sh.502000"
        assert fetcher._convert_stock_code("506000") == "sh.506000"
        assert fetcher._convert_stock_code("161116") == "sz.161116"
        # 18xxxx should fall through to default (sz), not match fund prefix
        result = fetcher._convert_stock_code("180003")
        assert result == "sz.180003", f"18xxxx should be sz, got {result}"

    def test_yfinance_routes_lof_to_ss(self):
        """YFinanceFetcher must route LOF 501/502/506 to .SS suffix."""
        from data_provider.yfinance_fetcher import YfinanceFetcher
        fetcher = YfinanceFetcher.__new__(YfinanceFetcher)
        assert fetcher._convert_stock_code("501018") == "501018.SS"
        assert fetcher._convert_stock_code("506000") == "506000.SS"
        assert fetcher._convert_stock_code("161116") == "161116.SZ"
