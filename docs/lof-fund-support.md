...
| 风险 | 影响范围 | 严重度 | 说明 |
|------|---------|--------|------|
| 18xxxx 从 ETF/FUND 移除 | 传统封闭式基金（已清盘/转型），fall through 到普通股票路径 | 极低 | 全栈一致移除（akshare_fetcher + base.py + search_service） |
| 519xxx 不匹配任何基金前缀 | 上证基金通系统基金，走普通股票路径 | 极低 | 非竞价系统交易 |
| 501/502/506 在 base/search/efinance 中新增 | 可能影响已有 ETF 路由逻辑 | 极低 | 7 个现有 ETF 测试全部通过，无回归 |
| efinance secid `1.501xxx` 可能返回空 | AkshareFetcher 失败时 efinance fallback 可能取不到 LOF 数据 | 低 | DataFetcherManager 会继续 fallback 到下一数据源（Baostock/YFinance） |
| LOF 被 fundamental_context 当基金跳过龙虎榜/资金流 | LOF 不会获取公司级数据 | 无 | 正确行为——基金不需要公司基本面数据 |
| fallback 路径 _fetch_etf_data 会再调一次 _enforce_rate_limit | 双重 rate limit | 无 | 两次 API 调用之间需要安全间隔 |
